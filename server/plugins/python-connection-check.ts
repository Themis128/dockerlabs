...
export default defineNitroPlugin(async (nitroApp) => {
  ...
  try {
    ...
    if (response && (response.status === 'healthy' || response.status === 'ok' || response.status === 'degraded')) {
      console.log(`[Nuxt Startup] ✓ Python backend connection verified at ${pythonServerUrl}`)
    } else {
      console.warn(`[Nuxt Startup] ⚠ Python backend responded but status is: ${response?.status || 'unknown'}`)
    }
  } catch (error: any) {
    ...
    if (isConnectionError) {
      ...
      console.warn(`[Nuxt Startup] ⚠ Nuxt will continue, but API calls may fail until Python server is available`)
    } else {
