...
def shutdown_pi(pi_number: int, username: str = DEFAULT_SSH_USERNAME,
    ...
    result = execute_ssh_command(
        ip, username, "sudo shutdown -h now", timeout=timeout
    )

    # Note: Shutdown command may disconnect before returning
    if result["success"] or "Connection reset" in result.get("error", ""):
        return {
            "success": True,
            "message": f"Shutdown command sent to Pi {pi_number} ({ip})",
            "pi": pi_number,
            "ip": ip,
        ...
        return {
            "success": False,
            "error": result.get("error", "Failed to shutdown Pi"),
            "pi": pi_number,
            "ip": ip,
        }
...
def main():
    parser = argparse.ArgumentParser(description="Shutdown Raspberry Pi")
