...
Since SSH password authentication is disabled, you need physical access to the Raspberry Pi to enable it or add your SSH key.
...
Copy the entire output (it starts with `ssh-rsa` and ends with `raspberry-pi-access`)
...
If you have physical access, this single command enables password authentication:
