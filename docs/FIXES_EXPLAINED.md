...
This document explains the two fixes applied to resolve Python server startup
issues.
...
When running Python server via `concurrently`, output was buffered and not
visible in real-time, making it hard to debug startup issues.
...
When running both servers with `npm run dev:all`, Nuxt would start immediately
and try to connect to Python backend before it was ready, causing connection
errors:
...
Added `wait-on` package to wait for Python server health endpoint before
starting Nuxt.
