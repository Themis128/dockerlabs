...
This document tracks the synchronization between the Nuxt frontend, Nuxt server routes, and Python backend.
...
- Frontend: `useApi` composable catches errors and returns `{success: false, error: string}`
- Nuxt Server Routes: Catch errors from Python backend and format them consistently
