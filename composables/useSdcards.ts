    ...
    try {
      ...
      if (response.success) {
        ...
        if (Array.isArray(sdcardsData)) {
          sdcardsStore.setSdcards(sdcardsData);
          notifications.success(`Loaded ${sdcardsData.length} SD card(s)`);
          return sdcardsData;
        } else {
          const error = 'Invalid response format - sdcards data not found';
